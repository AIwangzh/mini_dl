...
void AddGradFn::backward(const std::vector<float>& grad_out) {
    if (a_ && a_->requires_grad()) accumulate(a_, grad_out);
    if (b_ && b_->requires_grad()) accumulate(b_, grad_out);
}
...
std::vector<Tensor*> SubGradFn::parents() const { return {a_, b_}; }
...
std::vector<Tensor*> NegGradFn::parents() const { return {a_}; }